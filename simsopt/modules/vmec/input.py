...
class VmecInput:
    ...
    def read_boundary_input(self):
        ...
        Returns:
            ...
            zbs (float array): boundary harmonics for height coordinate
                (same size as xm and xn)
        """
        [xn_rbc,xm_rbc,rbc_input] = read_bounary_harmonic('rbc',self.input_filename)
        [xn_zbs,xm_zbs,zbs_input] = read_bounary_harmonic('zbs',self.input_filename)
    ...
    def position_first_derivatives(self, theta=None, zeta=None):
        ...
        dRdtheta = sine_IFT(self.xm,self.xn,float(self.nfp),theta,zeta,-self.xm*self.rbc)
        dzdtheta = cosine_IFT(self.xm,self.xn,float(self.nfp),theta,zeta,self.xm*self.zbs)
        dRdzeta = sine_IFT(self.xm,self.xn,float(self.nfp),theta,zeta,self.nfp*self.xn*self.rbc)
        dzdzeta = cosine_IFT(self.xm,self.xn,float(self.nfp),theta,zeta,-self.nfp*self.xn*self.zbs)
    ...
    def position_derivatives_surface(self, xm_sensitivity, xn_sensitivity, \
        ...
        dRdrmnc = np.cos(xm_sensitivity[:,np.newaxis,np.newaxis]*theta[np.newaxis,:,:] - \
                        self.nfp*xn_sensitivity[:,np.newaxis,np.newaxis]*zeta[np.newaxis,:,:])
        dZdzmns = np.sin(xm_sensitivity[:,np.newaxis,np.newaxis]*theta[np.newaxis,:,:] - \
                self.nfp*xn_sensitivity[:,np.newaxis,np.newaxis]*zeta[np.newaxis,:,:])
    ...
    def position_second_derivatives_surface(self, xm_sensitivity, xn_sensitivity, \
        ...
        d3Rdtheta2drmnc = -xm_sensitivity[:,np.newaxis,np.newaxis] * \
                           ...
                           np.cos(xm_sensitivity[:,np.newaxis,np.newaxis] * theta[np.newaxis,:,:] \
                           - self.nfp * xn_sensitivity[:,np.newaxis,np.newaxis] * zeta[np.newaxis,:,:])
        d3Zdtheta2dzmns = -xm_sensitivity[:,np.newaxis,np.newaxis] * \
                           xm_sensitivity[:,np.newaxis,np.newaxis] * \
                           np.sin(xm_sensitivity[:,np.newaxis,np.newaxis] * theta[np.newaxis,:,:] \
                           - self.nfp * xn_sensitivity[:,np.newaxis,np.newaxis] * zeta[np.newaxis,:,:])
