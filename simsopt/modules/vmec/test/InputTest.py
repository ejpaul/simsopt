...
class Test(unittest.TestCase):
    ...
    def test_area_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            ...
            dareadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc), epsilon=1e-8, \
                method='centered')
            dareadzmns_fd = finiteDifferenceDerivative(this_zbs,\
    ...
    def test_volume_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            dvolumedrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc), epsilon=1e-8, \
                method='centered')
            dvolumedzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs), epsilon=1e-8, \
                method='centered')
...
#                           theta = thetas, zeta = zetas)
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            d2Xdthetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[0], epsilon=epsilon, \
                method='centered')
            d2Xdzetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[1], epsilon=epsilon, \
                method='centered')
            d2Ydthetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[2], epsilon=epsilon, \
                method='centered')
            d2Ydzetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[3], epsilon=epsilon, \
                method='centered')
            d2Zdthetadzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[4], epsilon=epsilon, \
                method='centered')
            d2Zdzetadzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[5], epsilon=epsilon, \
                method='centered')
            d2Rdthetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[6], epsilon=epsilon, \
                method='centered')
            d2Rdzetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
...
#         self.assertTrue(np.all(np.shape(d2Zdthetadzeta == np.shape(thetas))))
        ...
        for itheta in range(0,self.vmecInput.ntheta,5):
            for izeta in range(0,self.vmecInput.nzeta,5):
                ...
                self.assertAlmostEqual(d2ydzeta2_fd, d2ydzeta2[izeta,itheta], 
                                       places=5)
                d2ydthetadzeta_fd = finiteDifferenceDerivative(this_theta,\
                ...
                self.assertAlmostEqual(d2Zdzeta2_fd, d2Zdzeta2[izeta,itheta], 
                                       places=5)     
                d2Zdthetadzeta_fd = finiteDifferenceDerivative(this_theta,\
    ...
    def test_position_second_derivatives_surface(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            d3Xdtheta2drmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[0], epsilon=1e-5, \
                method='centered')
            d3Ydtheta2drmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[3], epsilon=1e-5, \
                method='centered')
            d3Zdtheta2dzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[6], epsilon=1e-5, \
                method='centered')
            d3Xdzeta2drmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[1], epsilon=1e-5, \
                method='centered')
            d3Ydzeta2drmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[4], epsilon=1e-5, \
                method='centered')
            d3Zdzeta2dzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[7], epsilon=1e-5, \
                method='centered')
            d3Xdthetadzetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[2], epsilon=1e-5, \
                method='centered')
            d3Ydthetadzetadrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[5], epsilon=1e-5, \
                method='centered')
            d3Zdthetadzetadzmns_fd = finiteDifferenceDerivative(this_zbs,\
    ...
    def test_jacobian_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            djacobiandrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc), epsilon=1e-5, \
                method='centered')
            djacobiandzmns_fd = finiteDifferenceDerivative(this_zbs,\
    ...
    def test_normal_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            dNxdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[0], epsilon=1e-5, \
                method='centered')
            dNxdzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[0], epsilon=1e-5, \
                method='centered')
            dNydrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[1], epsilon=1e-5, \
                method='centered')
            dNydzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[1], epsilon=1e-5, \
                method='centered')
            dNzdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[2], epsilon=1e-5, \
                method='centered')
            for izeta in range(self.vmecInput.nzeta):
                for itheta in range(self.vmecInput.ntheta):
    ...
    def test_suface_curvature_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            dHdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[0], epsilon=epsilon, \
                method='centered')
            dHdzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[0], epsilon=epsilon, \
                method='centered')
            dKdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[1], epsilon=epsilon, \
                method='centered')
            dKdzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[1], epsilon=epsilon, \
                method='centered')
            dkappa1drmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[2], epsilon=epsilon, \
                method='centered')
            dkappa1dzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs)[2], epsilon=epsilon, \
                method='centered')
            dkappa2drmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[3], epsilon=epsilon, \
                method='centered')
            dkappa2dzmns_fd = finiteDifferenceDerivative(this_zbs,\
    ...
    def test_position_derivatives_surface(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            dXdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[0], epsilon=1e-5, \
                method='centered')
            dYdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[1], epsilon=1e-5, \
                method='centered')
            dRdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc)[3], epsilon=1e-5, \
                method='centered')
            dZdzmns_fd = finiteDifferenceDerivative(this_rbc,\
    ...
    def test_summed_radius_constraint_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            dconstraintdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc), epsilon=1e-8, \
                method='centered')
            dconstraintdzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs), epsilon=1e-8, \
                method='centered')
    ...
    def test_summed_proximity_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            dproximitydrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc), epsilon=1e-8, \
                method='centered')
            dproximitydzmns_fd = finiteDifferenceDerivative(this_zbs,\
                lambda zbs : temp_fun_zbs(im,zbs), epsilon=1e-8, \
                method='centered')
    ...
    def test_surface_curvature_metric_derivatives(self):
        ...
        for im in range(len(xm_sensitivity)):
            this_rbc = self.vmecInput.rbc[im]
            this_zbs = self.vmecInput.zbs[im]
            dmetricdrmnc_fd = finiteDifferenceDerivative(this_rbc,\
                lambda rbc : temp_fun_rbc(im,rbc), epsilon=epsilon, \
                method='centered')
            dmetricdzmns_fd = finiteDifferenceDerivative(this_zbs,\
